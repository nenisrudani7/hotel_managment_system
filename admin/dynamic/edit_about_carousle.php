...
?>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($_FILES["image"]["size"] > 500000) {
                    ...
                    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                            ...
                            $sql = "UPDATE about_heading_image SET image_path='img/$image_path' WHERE id=$id";
                            if (!mysqli_query($conn, $sql)) {
                                echo "Error updating image path: " . mysqli_error($conn);
                            } else {
                                ?>
                                <script>
                                    window.location.href="about_carousle.php";
                                </script>
                                <?php
                            }
                        } else {
        ...
        $sql = "UPDATE about_heading_image SET caption_heading='$caption_heading', caption_text='$caption_text' WHERE id=$id";
    ...
    if ($id_to_edit) {
        $sql = "SELECT * FROM about_heading_image WHERE id=$id_to_edit";
